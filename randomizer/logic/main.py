...
class GameWorld:
    ...
    def build_patch(self):
        ...
        if self.settings.is_flag_enabled(flags.ShuffleBowsersKeep):
            ...
            #patch twin rooms so that they always run event 2123 on load
            patch.add_data(0x20ED2E, [0x4B, 0x08])
            patch.add_data(0x20EDD3, [0x4B, 0x08])
            patch.add_data(0x20F0EE, [0x4B, 0x08])
            patch.add_data(0x20F2A4, [0x4B, 0x08])
            patch.add_data(0x20F38C, [0x4B, 0x08])
            patch.add_data(0x20F38F, [0x4B, 0x08])
            patch.add_data(0x20F392, [0x4B, 0x08])
            patch.add_data(0x20F395, [0x4B, 0x08])
            patch.add_data(0x20F398, [0x4B, 0x08])
            patch.add_data(0x20F39B, [0x4B, 0x08])
            patch.add_data(0x20F39E, [0x4B, 0x08])
            patch.add_data(0x20F3A1, [0x4B, 0x08])
            ...
            #modify event 332 for room chains
            ...
            # modify event 2121 to set tries counter to 10 at the start of each of the 6 starter rooms and then load their original entrance event
            event_patch_data = [0xA8, 0x2B, 0x0A, 0xC3, 0xE2, 0xD2, 0x01, 0xB3, 0x7A, 0xE2, 0xD2, 0x01, 0xB7, 0x7A, 0xE2, 0xD2, 0x01, 0xBB, 0x7A, 0xE2, 0xD2, 0x01, 0xBF, 0x7A, 0xE2, 0xD2, 0x01, 0xC3, 0x7A, 0xE2, 0xD2, 0x01, 0xC7, 0x7A, 0xD0, 0x1A, 0x0D, 0xFE, 0xD0, 0x1A, 0x0D, 0xFE, 0xD0, 0x1A, 0x0D, 0xFE, 0xD0, 0x1A, 0x0D, 0xFE, 0xD0, 0x1A, 0x0D, 0xFE, 0xD0, 0x1A, 0x0D, 0xFE]
            ...
            # modify event 2120 and 2123 to make unused rooms load and then run 1 of 2 events
            event_patch_data = [0xC3, 0xE2, 0x8C, 0x00, 0x8A, 0x7A, 0xE2, 0x9C, 0x00, 0x8A, 0x7A, 0xE2, 0xDA, 0x00, 0x8A, 0x7A, 0xE2, 0x00, 0x01, 0x8A, 0x7A, 0xE2, 0x24, 0x01, 0x8A, 0x7A, 0xE2, 0x25, 0x01, 0x8A, 0x7A, 0xE2, 0x26, 0x01, 0x8A, 0x7A, 0xE2, 0x27, 0x01, 0x8A, 0x7A, 0xE2, 0x28, 0x01, 0x8A, 0x7A, 0xE2, 0x29, 0x01, 0x8A, 0x7A, 0xE2, 0x2A, 0x01, 0x8A, 0x7A, 0xE2, 0x2B, 0x01, 0x8A, 0x7A, 0xD0, 0x4C, 0x01, 0xFE, 0xD0, 0x16, 0x0D]
            patch.add_data(0x1F7A4D, event_patch_data)
            i = 0x1F7A4D + len(event_patch_data)
            event_patch_data = [0x71, 0xC3, 0xD0, 0x48, 0x08, 0xFE]
            patch.add_data(0x1F7B1E, event_patch_data)
            i = 0x1F7B1E + len(event_patch_data)
            while i < 0x1F7B2E:
                patch.add_data(i, 0x9B)
                i += 1
            patch.add_data(0x1F7B2E, 0xFE)
            ...
            for index, door in enumerate(doors):
                ...
                patch.add_data(0x1f7AB4 + (index * 4), door[0].original_event)
                ...
                for j in range(0, len(door)):
                    ...
                    if j+1 == len(door):
                        if door[j].is_final:
                            ...
                            patch.add_data(door[j].event_2120_location + 1, [0x8E, 0x7A])
